...
function showAppUI(user = null) {
  ...
  // Show user information if provided
  if (user && userInfo) {
...
function logout() {
  ...
  chrome.storage.local.remove(['authenticated', 'user', 'accessToken'], () => {
    ...
    // Update status
    updateStatus('Signed out', '#6b7280');
  });
  
  // Optional: Call backend logout endpoint for cleanup (don't wait for response)
  chrome.storage.local.get(['accessToken'], (result) => {
    if (result.accessToken) {
      fetch('http://localhost:3000/api/auth/logout', {
        ...
        // Ignore errors - extension already logged out locally
        console.log('Backend logout cleanup failed:', error);
      });
    }
  });
}
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        if (response.success) {
          ...
          // Get user data from storage and show app UI
          chrome.storage.local.get(['user'], (result) => {
            showAppUI(result.user);
          });
          startPeriodicUpdates();
          startStreamingUpdates();
